...
Domain Name				Mapping
example.com				default homepage (no remapping)
www.example.com			default homepage (no remapping)
test-page.example.com	www.example.com/test-page/
mypost.example.com		www.example.com/mypost/

			
== Installation ==

1. Upload `automatic-subdomains` directory to the `/wp-content/plugins/` directory
...
No. You must first edit your zone file to create the subdomains or a wildcard subdomain to resolve to your WordPress site.
...
Subdomains will map to Pages and Posts based on the post_name field in the database. The subdomain must match the post_name exactly in order to map, or it will redirect to the homepage.

= What if I have a fourth, fifth, etc. level domain? =

The mapping will be made based on the deepest (leftmost) element of the full domain name.
