...
function wpelf_activation() {
	...
	if (get_option('wpelf_disabled') === false) {
		add_option('wpelf_disabled', serialize(array()), __('List of not allowed commands', 'elfinder'));
	}
	...
	if (get_option('wpelf_upload_allow') === false) {
		add_option('wpelf_upload_allow', serialize(array('images/*')), __('List of mime-types allowed to upload. Can be set exactly image/jpeg or to group application', 'elfinder'));
	}
	if (get_option('wpelf_upload_deny') === false) {
		add_option('wpelf_upload_deny', serialize(array('all')), __('List of mime-types disallowed to upload', 'elfinder'));
	}
	...
	if (get_option('wpelf_defaults') === false) {
		add_option('wpelf_defaults', serialize(array( 'read' => true, 'write' => true, 'rm' => true )), __('Default access for files/directories. Default: array( "read" => true, "write" => true, "rm" => true )', 'elfinder'));
	}
	if (get_option('wpelf_perms') === false) {
		add_option('wpelf_perms', serialize(array()), __('Individual folders/files permisions', 'elfinder'));
	}
	...
	if (get_option('wpelf_archive_mimes') === false) {
		add_option('wpelf_archive_mimes', serialize(array()), __('List of file archives allowed to create. If not set will allow all detected archvies', 'elfinder'));
	}
	if (get_option('wpelf_archivers') === false) {
		add_option('wpelf_archivers', serialize(array()), __('Info about archivers to use. See example below. Leave empty for auto detect', 'elfinder'));
	}
    // Example:
	//
	// 'archivers' => array(
