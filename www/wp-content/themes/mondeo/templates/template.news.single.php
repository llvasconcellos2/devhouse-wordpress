...
//Settings
	global $shortname;
	$layout = get_option($shortname.'_news_layout_single');
	$author = get_option($shortname.'_news_post_byline_author') == 'true';
	$date = get_option($shortname.'_news_post_byline_date') == 'true';
	$comments = get_option($shortname.'_news_post_byline_comments') == 'true';
	$categories = get_option($shortname.'_news_post_byline_categories') == 'true';
	$author_info = get_option($shortname.'_news_author_info_display') == 'true';
	if($layout == 'left side') {
		$class = 'left-side';
...
<section class="post-header">
	...
	<div class="entry-meta">
		...
		<?php if($categories): ?><span><?php _e('In','hawktheme'); ?> <?php echo get_the_term_list( $post->ID, 'news', '', ',', '' ); ?></span><?php endif; ?>
...
<?php echo get_the_term_list( $post->ID, 'news-tag', '<div class="tags">'. __('Tagged with: ','hawktheme'), ' , ', '</div>' ); ?>
<?php ht_the_author($author_info, 'news-item'); ?>
</div><!--end post single-->
