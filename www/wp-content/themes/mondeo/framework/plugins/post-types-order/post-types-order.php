...
define('CPTPATH', FrameWork. '/plugins/post-types-order');
define('CPTURL', FrameWork_Url. '/plugins/post-types-order');
...
function cpto_get_previous_post_where($where)
    {
		...
		$in_same_cat_isset = isset( $in_same_cat ) ? $in_same_cat: ''; 
        if ( $in_same_cat_isset || !empty($excluded_categories) ) 
            {
...
function cpto_get_previous_post_sort($sort)
    {
		...
		$posts_in_ex_cats_sql_isset = isset( $posts_in_ex_cats_sql ) ? $posts_in_ex_cats_sql: ''; 
        
        $query = "SELECT p.* FROM $wpdb->posts AS p
                    WHERE p.menu_order < '".$current_menu_order."' AND p.post_type = '". $post->post_type ."' AND p.post_status = 'publish' $posts_in_ex_cats_sql_isset";
        $results = $wpdb->get_results($query);
                ...
                foreach( $post_types as $post_type_name ) 
                    {
                        ...
                        if ($post_type_name == 'slider-item')
                            add_submenu_page('edit.php?post_type=slider-item', 'Re Order', 'Re Order', 'administrator', 'order-post-types-slider-item', array(&$this, 'SortPage') );
